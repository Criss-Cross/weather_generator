...
test_rainfallPerMonth <- c()
for (i in 1:12){
  temp<- train_data$Rainfall.amount..millimetres.[train_data$state == "wet" & train_data$Month == i]
  test_rainfallPerMonth <- c(test_rainfallPerMonth, list(temp))
  rm(temp)
}
...
#p-val = 0.00016, therefore difference between average rainfall for each 
...
# missing a day from October and December
table(rain2019$Day)
...
```
```{r}
# to perform Wilcoxon Signed Rank test, remove those dates from testSeasonalRainfall10
...
simRainfall2019 <- testSeasonalRainfall10[-c(278, 353)] # remove day 278 and day 353 data drom testSeasonalRainfall10
```
...
# as the p-value = 0.9695, it can be concluded that there is no significant diference between the simulated 2019 data and the actual 2019 data. This means that the data is not consistently too high or too low. 
