...
Both of the trends, that the generator overstimates 2019 and underestimates 2011, are expected. The generator was trained on data from 2009 to 20018, so the simulated data should have an average close to the avergae of those 10 years. To confirm this we simulated 10 years of data, 2008-2019, 100 times, finding the average rainfall per year of each simulation, and then compared that to the actual average rainfall per year of 2008-2019. The two are more different than we would like, but the difference is still < 1%. The variation in monthly rainfall across the 10 years possibly resulted in inaccurate monthly probabilities, for transitioning between states and the rainfall distribution. On an individual basis, the simulations are usually not significantly different, so they are sufficient for rough estimation and analysis. 
...
par(mfrow = c(2,1))
barplot(SumSim)
barplot(rain2011$Rainfall.amount..millimetres.)
```
...
rainfallAvgMonth <- rainMonthDataCalc(train10_data_2)# retraining seasonal generator probTransitionStatesMonthsFrame with train10_data
