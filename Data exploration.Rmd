...
generateRainfallSeasonal <- function(input){
  ...
  for (i5 in 1:dim(input)[1]){
    ...
    if (currentState == "wet"){
      ...
      rainAmount <- rainfallAvgMonth[input[i5, 'Month']]*exp(rnorm(1))/1.648 #average of rnorm(exp(1)) is ~1.648, so we divide by that so the average of each month will just be the actual average calculated from rainfallAvgMonth.
