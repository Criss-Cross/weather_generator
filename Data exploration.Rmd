...
- Tested model on secondary data source and compared the respective results
...
```{r}
plot_seasonal_data <- function(training_data_input,seasoned_data_rainfall){
  par(mfrow=c(2,1))
  barplot(training_data_input$Rainfall.amount..millimetres., ylim = c(0,80), main = "Original Data", ylab = "Rainfall (mm)", xlab = "Index")
  barplot(seasoned_data_rainfall, ylim = c(0,80), main = "Simulated Data - seasonal", ylab = "Rainfall (mm)", xlab = "Index")
}
plot_seasonal_data(train_data,testSeasonalRainfall)
```
...
So far, our generator utilised a two-state Markov chain based on whether a day was wet or dry. The simulations seem to be accurate, at least in regards to the wet dry cycle, but there is still definite room for improvement. A possible avenue for improvement is investigating if there are consistent patterns at larger time intervals, such as weeks and months. If such patterns are present they could guide the total volume of rainfall for that time period or be an additional layer of Markov Chain to standardise or correct the first, that is based on days. Rainfall by weeks will be explored first. 
```{r}
...
# To perform the Wilcoxon Signed Rank test we must have paired data, so we have to find the extra dates in testSeasonalRainfall10 (note: 2019 is a non leap year) and remove them.
# Upon review we find that there are days missing from October and December.
```
...
# Therefore 5-10-2019 would be row 278 (277+1)
which(rain2019$date == "18-12-2019") # find what row the day before 5-10-2019 is
# Therefore 5-10-2019 would be row 353 (351+2). Add two as both 5th october and 19th december are missing
```

```{r}
# We find that the days are day 278 and 353 from testSeasonalRainfall10
simRainfall2019 <- testSeasonalRainfall10
```
...
monthList <- monthList['Month']
testSeasonalRainfall10 <- generateRainfallSeasonal(monthList)
simRainfall2019 <- testSeasonalRainfall10 # Have to remove day 278 and day 353 data from testSeasonalRainfall10 because 2019 data list is missing these days
```

```{r}
wilcox <- function(seed, year, remove_days){
  set.seed(seed)
  monthList <- subset(weather_data, Year == year) # a year that isn't a leap year to get the month list, as generation algorithm only requires months
  monthList <- monthList['Month']
  testSeasonalRainfall10 <- generateRainfallSeasonal(monthList)
  if (remove_days == 1){
    simRainfall2019 <- testSeasonalRainfall10[-c(278, 353)]
  } else if (remove_days == 0){
  ...
  print(wilcox.test(rain2019$Rainfall.amount..millimetres., simRainfall2019, paired = T, alternative = "t", conf.int = T, conf.level = 0.95))
  WP <- wilcox.test(rain2019$Rainfall.amount..millimetres., simRainfall2019, paired = T, alternative = "t", conf.int = T, conf.level = 0.95)
...
```{r, echo=FALSE}
wilcox(12345, 2011, 1)
wilcox(347876, 2011, 1)
wilcox(3456789, 2011, 1)
```
...
weather_data$date2 <- do.call(paste, c(weather_data[c("Day", "Month", "Year")], sep = "-")) # creating a column that contains the date for easy indentification
