...
In accordance with the literature, in order to increase the accuracy of the model, seasonality (the probability of the transition states) for specific months was taken into consideration. The Log-normal distribution was used in the generator as it produced a very similar shape to the actual data. Following this, we tested our generator by generating an alternative rainfall pattern for 2019 (using the previous decade's rainfall levels as training data) and then compared this result to the actual. We then proceeded to run several statistical tests over this generated year (most notably the wilcoxon signed ranked test) and found our generated year to not be very significantly different from the year's actual rainfall levels. Furthermore we tested the model against a different data source to verify its reliability and consistency. Concluding that we were able to create an accurate weather simulation of rainfall (based on a given data source). 
...
Another one of the earliest works done on this topic was done by Quetelet in 1852 (Katz, 1985), where he showed that consecutive runs of wet or dry days show persistence. Since then, precipitation has been a topic of intense research as the absence or presence of rainfall can affect a large number of other variables that need to be simulated and modelled for weather. It is vital in areas such as farming, ecosystem, climate change simulations, and other important processes (Wilks and Wilby, 1999).
...
- Testing the model on secondary data source and comparing the results to confirm that it is consistent 
...
```
Now that we have the state for every observation in the data set, we can now find the transition states. We do this by looping through the data set, and record the transition state as the state of the previous observation to the new observation. This results in four possible states: Dry to dry, dry to wet, wet to wet, and wet to dry. The first observation's transition state is not found as there is no state from the day before to calculate it. We then record this as a column of the data set.
```{r}
...
```
```{r}
table(train_data$Month, train_data$Year, dnn = c("Month", "Year")) # only four months of data in 2020
```
...
mean(p2)
```
Testing the basic generator, it appears that the amount of rainfall when it rained approximated the actual case very well. However, comparing 3761 days of simulated rainfall compared to actual rainfall, we can see that the actual rainfall comes more in waves, compared to the simulated rainfall here. This is because it rains more in winter months, meaning that the transition state probabilities are different for each month or season.
```{r, fig.height=8}
...
Therefore, we need to change the probability for the state changes in the generator depending on the month of the year. To do this we created a data frame to count the number of occurrences of transition states for each month. We initially filled the data frames all with zeroes, and increased the count by one for every occurrence.
...
```{r}
#Calculates the transition probabilities for each montht
trainGenerator <- function(input_data){
  ...
  if((transitonStatesMonthsFrame[i, 'DryToDry'] + transitonStatesMonthsFrame[i, 'DryToWet']) != 0){
  ...
  } else { # probability will be NaN for both (as will be dividing by 0), so set to 0 
      probTransitonStatesMonthsFrame[i, 'DryToDry'] <- 0
      probTransitonStatesMonthsFrame[i, 'DryToWet'] <- 0
  }

  if((transitonStatesMonthsFrame[i, 'WetToDry'] + transitonStatesMonthsFrame[i, 'WetToWet']) != 0){
    ...
    probTransitonStatesMonthsFrame[i, 'WetToDry'] <- 0
    probTransitonStatesMonthsFrame[i, 'WetToWet'] <- 0
  }
...
```{r}
probTransitonStatesMonthsFrame <- trainGenerator(train_data)
...
```{r}
#calculating the mean rainfall when it rains for each month, to change how much it rains according to the generator later on. Limited to less than 70 mls as the dataset is not large enough for large rainfall amounts (such as 114ml in feburary) to not throw the mean off.
rainMonthDataCalc <- function(input_data){
  rainMonthData <- c()
  for (i in 1:12){
    temp <- input_data$Rainfall.amount..millimetres.[input_data$state == "wet" & input_data$Month == i & input_data$Rainfall.amount..millimetres. < 70]
    if(length(temp) == 0){ # if no wet days in month, will be empty vector of numeric(0)
      temp <- c(0)
    }
  ...
  return(rainfallAvgMonth)
}

```

```{r}
rainfallAvgMonth <- rainMonthDataCalc(train_data)
...
```
Simulated rainfall now follows a more seasonal pattern, as seen in the original data. Since we have used the original data transition probabilities and rainfall averages, we expect that our model can give similar results to the original data.
```{r}
...
```{r}
probTransitonStatesMonthsFrame <- trainGenerator(train_data) # retraining seasonal generator probTransitionStatesMonthsFrame with new training data (now only 80% of the original train_data)
rainfallAvgMonth <- rainMonthDataCalc(train_data) # calucating the new average rainfall for each month
```

The newly calibrated seasonal generator was then used to predict the testing data. This just involved generating simulated rainfall with the same number of observations as the testing data (about 2.6 years of rainfall, or 20% of the original 10.5 years)
...
```{r}
probTransitonStatesMonthsFrame <- trainGenerator(train10_data)
rainfallAvgMonth <- rainMonthDataCalc(train10_data) # retraining seasonal generator probTransitionStatesMonthsFrame with train10_data
set.seed(10356)
...
par(mfrow = c(2,1))
barplot(data_2019$Rainfall.amount..millimetres., xlab = "Index", ylab = "Rainfall (mm)", main = "Actual 2019 Rainfall (Perth)")
barplot(testSeasonalRainfall10, xlab = "Index", ylab = "Rainfall (mm)", main = "Simulated Rainfall (Perth)")
```
...
rm(rainSort_2019)
```
From the above qq-plots, it is clear that the data is not normally distributed, whether we include or exclude dry days. Thus, we will use the non-parametric Wilcoxon Signed Rank test to check the similarity between the simulated rainfall for 2019 and the actual rainfall. The null-hypothesis will be that the two data sets, simulated and actual, are not different. The alternative hypothesis will be that they are different.
...
Running the test on the 2019 rainfall and a simulated 2019 rainfall gave a p-value of 0.0402. This is less than the significance level of 0.05, and this it can be concluded that the two are different. Looking at the sum of ranks of differences, the simulation significantly overestimated, as there are far more positive differences. However, the p-value is only just under the 0.05 level of significance.
...
Given that three out of the four tests returned a p-value greater than the level of significance, and the only result less than 0.05 was by a small amount, it can be concluded that the generator will usually produce results that are not significantly different from the actual data.

An interesting observation is that all four tests returned higher positive sum rank differences than negative, and often by a large margin. This implies that the generator is generally overestimating the actual data. 
...
require(ggplot2)
ggplot(train_data, aes(x = factor(Year), y = Rainfall.amount..millimetres.)) +
  geom_bar(stat = "identity") + 
  xlab("Year") + 
  ylab("Rainfall (mm)") +
  ggtitle("Rainfall (mm) by year")
...
```
A likely explanation for this is that 2019 had a below average total rainfall when compared to the other 10 years. In order to confirm this, we will perform a Wilcoxon Signed Rank test on 2011, which has an above average rainfall, to see if there is a larger number of negative rank sum differences.,

```{r}
# looking at 2013 data (which had a larger than average amount of rain)
wilcox(654321, 2011) # wilcoxon test on 2013
set.seed(654321)
rain2011 <- subset(weather_data, Year == 2011) # A year that isn't a leap year to get the month list, as generation algorithm only requires months
simRainfall2011 <- generateRainfallSeasonal(rain2011)
par(mfrow = c(2,1))
barplot(simRainfall2011, xlab = "Index", ylab = "Rainfall (mm)", main = "Simulated Rainfall")
barplot(rain2011$Rainfall.amount..millimetres., xlab = "Index", ylab = "Rainfall (mm)", main = "2011 Rainfall")
rm(rain2011)
rm(simRainfall2011)

```
Through the barplots and the p-value, we can see that the simulation and the actual data are not significantly differennt, but it can also be seen that there is a much more negative sum rank differences than positive, supporting our hypothesis. To confirm that this result was not mere chance, we ran the test two more times, with different simulated data, and both had had more negative differences. 
...
Both of the trends, that the generator overstimates 2019 and underestimates 2011, are expected. The generator was trained on data from 2009 to 2018, so the simulated data should have an average close to the avergae of those 10 years. To confirm this we simulated 10 years of data, 2008-2019, 100 times, finding the average rainfall per year of each simulation, and then compared that to the actual average rainfall per year of 2008-2019. The two are more different than we would like, but the difference is still < 1%. A reason for this is that when training the generator, extreme rainfall (>70mm) is not included in order to make the simulations more realistic. This means that the generator will underestimate years with extreme rainfall, so it is expected tha the simulated rainfall will be less than the actual to a degree. Additionally, the variation in monthly rainfall across the 10 years possibly resulted in inaccurate monthly probabilities, for transitioning between states and the rainfall distribution. On an individual basis, the simulations are usually not significantly different, so they are sufficient for rough estimation and analysis. 

```{r}
probTransitonStatesMonthsFrame <- trainGenerator(train10_data)
rainfallAvgMonth <- rainMonthDataCalc(train10_data) # ensure generator is traind on train10_data

simSum = 0
trials <- 100
for(i in c(1:trials)){
   set.seed(i) 
  simulation <- generateRainfallSeasonal(train10_data) # generate simulation same length as train10_data
  simSum = simSum + sum(simulation)/10 # get average rainfall per year over simulated 10 years
}
meanSumSim <- simSum/trials # average rainfall per year from 'trial' number of simulations
meanSumActual <- sum(train10_data$Rainfall.amount..millimetres.)/10 # actual average rainfall per year
cat(meanSumSim, "is the simulated average rainfall per year \n")
cat(meanSumActual, "is the actual average rainfall per year \n")
```

Below are the barplots of an aggregation of 100 simulations of 2018 and the actual 2018 rainfall. The simulated rainfall is low in the earlier months, as the 114mm of rainfall in February was not included in the training. As a result, the  of the simulated mean is almost 100mm below the actual mean.
```{r, message = FALSE, warning=FALSE}
rain2011 <- subset(train_data, Year == 2018)
probTransitonStatesMonthsFrame <- trainGenerator(rain2011)
rainfallAvgMonth <- rainMonthDataCalc(rain2011) # ensure generator is traind on train10_data

simTot <- rep(0, length(rain2011))
simSum <- 0
trials <- 100
for(i in c(1:trials)){
  set.seed(i) 
  simulation <- generateRainfallSeasonal(rain2011) # generate simulation same length as train10_data
  simTot = simTot + simulation # get average rainfall per year over simulated 10 years
  simSum = simSum + sum(simulation)
}
SumSim <- simTot/trials # average rainfall per year from 'trial' number of simulations
meanSumSim <- simSum/trials
meanSumActual <- sum(rain2011$Rainfall.amount..millimetres.) # actual average rainfall per year
cat("\n", meanSumSim, "is the simulated rainfall \n")
cat("", meanSumActual, "is the actual rainfall \n")
par(mfrow = c(2,1))
barplot(SumSim, main = "100 Aggregated simulations of 2018 rainfall(mm)", xlab = "Index", ylab = "Rainfall (mm)")
barplot(rain2011$Rainfall.amount..millimetres., main = "Actuall 2018 rainfall(mm)", xlab = "Index", ylab = "Rainfall (mm)")
```

```{r}
month <- 8
year <- 2018
mean(train_data$Rainfall.amount..millimetres.[train_data$Month==month & train_data$Year == year & train_data$Rainfall.amount..millimetres. > 1])
rainfallAvgMonth[month]
```
...
probTransitonStatesMonthsFrame <- trainGenerator(train10_data2)
rainfallAvgMonth <- rainMonthDataCalc(train10_data2)# retraining seasonal generator probTransitionStatesMonthsFrame with train10_data
...
rainYear <- subset(weather_data2, Year == 2019)
print(wilcox.test(rainYear$Rainfall.amount..millimetres., testSeasonalRainfall10_2, paired = T, alternative = "t", conf.int = T, conf.level = 0.95))
WP <- wilcox.test(rainYear$Rainfall.amount..millimetres., testSeasonalRainfall10_2, paired = T, alternative = "t", conf.int = T, conf.level = 0.95)
cat("P-value is",WP$p.value,"\n")
```
...
par(mfrow = c(2,1))
barplot(testSeasonalRainfall10_2, xlab = "Index", ylab = "Rainfall (mm)", main = "Simulated Rainfall")
barplot(data_2019_2$Rainfall.amount..millimetres., xlab = "Index", ylab = "Rainfall (mm)", main = "2019 Rainfall")
# Conclusion and summary of the findings:

par(mfrow = c(2,2))
barplot(data_2019$Rainfall.amount..millimetres., xlab = "Index", ylab = "Rainfall (mm)", main = "Actual 2019 Rainfall (Perth)")
barplot(data_2019_2$Rainfall.amount..millimetres., xlab = "Index", ylab = "Rainfall (mm)", main = "Actual 2019 Rainfall (Sydney)")
barplot(testSeasonalRainfall10, xlab = "Index", ylab = "Rainfall (mm)", main = "Simulated Rainfall (Perth)")
barplot(testSeasonalRainfall10_2, xlab = "Index", ylab = "Rainfall (mm)", main = "Simulated Rainfall (Sydney)")
```
# Conclusion and summary of the findings:

In brief, our findings indicate that the simulated weather created by our generator is not statistically different from the actual rainfall data. More specifically, since our p-values are relatively high, this indicates that there is no statistically significant difference between our generated results and the actual results. Despite the results not being a completely accurate prediction of 2019 (as seen in the bar plots above), the simulation both follows an accurate distribution of rainfall and has a median that is similar to the actual rainfall. 
