...
```{r}
train_data <- subset(weather_data, Year >= 2010) # only look at the mos recent, relevant data.
library(ggplot2)
...
par(mfrow = c(4,1))
plot(RainfallWeeklyDiff[c(1:350), 2], RainfallWeeklyDiff[c(1:350), 1], xlab = "week", ylab = "change in rainfall (mm)", cex = 2, type = "b", main = "Difference in Rainfall (mm) across weeks")
plot(RainfallWeekly[c(1:350), 2], RainfallWeekly[c(1:350), 1], xlab = "week", ylab = "rainfall (mm)", type = "l", main = "Rainfall (mm) by week")
plot(RainfallWeeklyDiff[c(351:700), 2], RainfallWeeklyDiff[c(351:700), 1], xlab = "week", ylab = "change in rainfall (mm)", cex = 2, type = "b", main = "Difference in Rainfall (mm) across weeks")
plot(RainfallWeekly[c(351:700), 2], RainfallWeekly[c(351:700), 1], xlab = "week", ylab = "rainfall (mm)", type = "l", main = "Rainfall (mm) by week")


# the seasonal trends can be clearly seen through these plots, however there does not seem to be any calear patterns in the wet weeks. As such, a simulator using weeks will not be explored.
```
...
for(i in c(1:100)){
  set.seed(i)
  tempRainfall <- generateRainfallSeasonal(monthList)
  sumRainfall <- sumRainfall + sum(tempRainfall)
}
meanRainfallSim <- sumRainfall/100
meanRainfallSim

meanRainfallActual <- sum(train10_data$Rainfall.amount..millimetres.)/10
meanRainfallActual
```
