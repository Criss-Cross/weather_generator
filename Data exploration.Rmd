...
# Introduction and problem statement:
 The purpose of this project is to develop a weather generator for rainfall using the data gathered from the Perth Metro weather station (9225). To do so, we will be using a two-state Markov chain to predict the rainfall occurrences. Since we are creating a weather generator, the distinction must first be made between a 'weather generator' and a 'weather predictor' or forecast. The primary difference between the two is that a weather generator is a stochastic model. In other words, the simulated weather sequences produced by our generator will be different to past or future weather predictions or observations made using a forecasting model because our weather generator mimics weather data and is not an accurate prediction of the weather. 
...
>>>>>>> 65e8fb428cf64a6050c7e1a3ba2e75d1bafc7704
...
##### Rainfall amount (This follows the literature of George, Letha, and Jairaj 2016).
1. Exploratory Data Analysis:
...
#calculating the number of occurances of each trainsition state for each month
...
>>>>>>> b8d074c2640bb8a56ef44ce6abc3fac905150a21
...
To check if the simulated year is a good prediction of 2019 specifically, we plotted it against several years that were used to train it. As the simulation was created using data from those years, it should show some resemblance, but hopefully the resemblance will be less than or equal to the similarity of the simulation with the 2019 data, as 
```{r}
...
```
```{r}
# after finding the days that need to be removed we find that the days are day 278 and 353 from testSeasonalRainfall10
